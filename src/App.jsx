...
import { X, ArrowUpRight, Zap, Send, Layers, Layout, ArrowRight, Code, Cpu, Search, ExternalLink, Quote } from 'lucide-react';

// ─── palette
const C = {
  bg:'#0a0a0a', surface:'#111111', logo:'#f0ede8',
  textDim:'rgba(255,255,255,0.28)', textMid:'rgba(255,255,255,0.50)', textHi:'rgba(255,255,255,0.88)',
  accent:'#e8e4dc', glint:'#c8a97a',
};
...
// ─── shared styles
const pill = { display:'inline-flex', alignItems:'center', gap:5, padding:'3px 10px', borderRadius:100, border:'1px solid rgba(255,255,255,0.07)', fontFamily:"'DM Mono',monospace", fontSize:8, letterSpacing:'0.13em', textTransform:'uppercase', color:'rgba(255,255,255,0.35)' };
const rowBase = { display:'flex', alignItems:'center', justifyContent:'space-between', padding:'18px 20px', borderRadius:16, border:'1px solid rgba(255,255,255,0.04)', background:'rgba(255,255,255,0.015)', cursor:'pointer', transition:'all 0.25s', textDecoration:'none', color:'inherit' };
const onRH = (e) => { e.currentTarget.style.background='rgba(255,255,255,0.04)'; e.currentTarget.style.borderColor='rgba(255,255,255,0.09)'; };
const onRL = (e) => { e.currentTarget.style.background='rgba(255,255,255,0.015)'; e.currentTarget.style.borderColor='rgba(255,255,255,0.04)'; };

// ─── image with fallback
function ProjectImg({ src, fallback, style }) {
  const [err, setErr] = useState(false);
  if (err) return <div style={{ ...style, background:fallback }} />;
  return <img src={src} alt="" style={style} onError={() => setErr(true)} />;
}

// ─── desktop popup content
function PopupContent({ blockId, expandedDemo, setExpandedDemo }) {
  switch (blockId) {
    case 'work': return (
      <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%' }}>
        {demos.map(d => (
          <div key={d.id}>
            <div style={{ ...rowBase, flexDirection:'column', alignItems:'stretch', padding:0, overflow:'hidden' }}
              onClick={() => setExpandedDemo(expandedDemo === d.id ? null : d.id)}
              onMouseEnter={onRH} onMouseLeave={onRL}>
              ...
              <div style={{ maxHeight: expandedDemo===d.id ? 130 : 0, overflow:'hidden', transition:'max-height 0.4s cubic-bezier(0.19,1,0.22,1)' }}>
                <div style={{ padding:'0 20px 20px', borderTop:'1px solid rgba(255,255,255,0.04)' }}>
                  <p style={{ fontSize:13, color:C.textDim, lineHeight:1.7, margin:'14px 0 12px' }}>A bespoke digital presence crafted for {d.name}. Focused on performance, conversion, and brand alignment from the ground up.</p>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%' }}>
        <div style={{ padding:'18px 20px', borderRadius:16, border:'1px solid rgba(255,255,255,0.06)', background:'rgba(255,255,255,0.02)', marginBottom:4 }}>
          <p style={{ fontFamily:"'DM Mono',monospace", fontSize:8, letterSpacing:'0.13em', textTransform:'uppercase', color:C.textMid, display:'flex', alignItems:'center', gap:8, margin:'0 0 8px' }}>
            <span style={{ width:5, height:5, borderRadius:'50%', background:'rgba(255,255,255,0.5)', display:'inline-block' }} /> Available — Q2 2025
          </p>
          <p style={{ color:C.textDim, fontSize:13, margin:0, lineHeight:1.6 }}>Direct responses within 2 hours. No agencies, no delays.</p>
        </div>
        {contacts.map(c => (
          <a key={c.id} href={c.href} target="_blank" rel="noopener noreferrer" style={rowBase} onMouseEnter={onRH} onMouseLeave={onRL}>
            <div style={{ display:'flex', alignItems:'center', gap:14 }}>
              <div style={{ width:34, height:34, borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', background:`${c.brandColor}18`, border:`1px solid ${c.brandColor}30`, flexShrink:0 }}>
                <svg viewBox="0 0 24 24" fill="currentColor" style={{ width:14, height:14, color:c.brandColor }}><path d={c.path}/></svg>
              </div>
              <div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%' }}>
        {services.map((s,i) => (
          <div key={i} style={{ display:'flex', gap:14, padding:'18px 20px', borderRadius:16, border:'1px solid rgba(255,255,255,0.04)', background:'rgba(255,255,255,0.015)' }}>
            <div style={{ width:34, height:34, borderRadius:9, display:'flex', alignItems:'center', justifyContent:'center', background:`${C.glint}14`, border:`1px solid ${C.glint}25`, color:C.glint, flexShrink:0 }}>{s.icon}</div>
            <div>
    ...
    case 'about': return (
      <div style={{ display:'flex', flexDirection:'column', gap:16, width:'100%' }}>
        <div style={{ padding:'28px', borderRadius:16, border:'1px solid rgba(255,255,255,0.05)', background:'rgba(255,255,255,0.02)', position:'relative', overflow:'hidden' }}>
          ...
          <p style={{ fontFamily:"'Godber',sans-serif", fontSize:32, color:C.textHi, lineHeight:1.2, margin:0, position:'relative', zIndex:1 }}>We build fast.<br/>We build right.<br/>That's it.</p>
        </div>
        <p style={{ fontSize:13, color:C.textDim, lineHeight:1.75, margin:0, padding:'0 4px' }}>Arcodic is a Cape Town–based web studio. We work with brands that care about craft — delivering precision-built digital experiences on timelines that don't waste anyone's time.</p>
      </div>
    );
    case 'speed': return (
      <div style={{ display:'flex', flexDirection:'column', gap:14, width:'100%' }}>
        ...
        <p style={{ fontSize:13, color:C.textDim, lineHeight:1.75, margin:0 }}>From brief to live site in a single business day. No compromises on quality — we've built the systems to move fast without breaking things.</p>
      </div>
    );
    case 'social': return (
      <div style={{ display:'flex', flexDirection:'column', gap:10, width:'100%' }}>
...
export default function App() {
  ...
  const blocks = [
    ...
    { id:'about',   isAbout:true,   pos:{ top:'39vh',   left:'27vw',  width:'11vw', height:'30vh' }, label:'ABOUT',     title:'About' },
  ...
  const closeAll  = () => { setActiveBlock(null); setExpandedDemo(null); };

  return (
    <div className="app-root" style={{ width:'100%', background:C.bg, color:'#fff', fontFamily:"'DM Sans',sans-serif" }}>

      {/* ── Status bar ── */}
      <div style={{ position:'fixed', top:20, left:'50%', transform:'translateX(-50%)', zIndex:200, display:'flex', alignItems:'center', gap:18, padding:'7px 18px', borderRadius:100, border:'1px solid rgba(255,255,255,0.06)', background:'rgba(255,255,255,0.03)', backdropFilter:'blur(20px)', whiteSpace:'nowrap' }}>
      ...
      <div className="mobile-only">
        <div style={{ padding:'72px 16px 48px', display:'flex', flexDirection:'column', gap:12 }}>
          ...
          <div style={{ borderRadius:16, background:C.surface, border:'1px solid rgba(255,255,255,0.05)', padding:'32px 28px', position:'relative', overflow:'hidden' }}>
            <Quote size={48} style={{ position:'absolute', top:14, right:14, color:'rgba(255,255,255,0.04)', zIndex:0 }} />
            <p style={{ fontFamily:"'Godber',sans-serif", fontSize:36, color:C.textHi, lineHeight:1.25, margin:0, position:'relative', zIndex:1 }}>We build fast.<br/>We build right.<br/>That's it.</p>
          ...
          <div style={{ borderRadius:16, background:C.surface, border:'1px solid rgba(255,255,255,0.05)', overflow:'hidden' }}>
            ...
            {demos.map(d => (
              <a key={d.id} href={d.href} target="_blank" rel="noopener noreferrer"
                style={{ display:'flex', alignItems:'center', gap:14, padding:'12px 20px', borderTop:'1px solid rgba(255,255,255,0.04)', textDecoration:'none', color:'inherit' }}>
          ...
          <div style={{ borderRadius:16, background:C.surface, border:'1px solid rgba(255,255,255,0.05)', padding:'20px' }}>
            <span style={{ ...pill, marginBottom:12, display:'inline-flex' }}><Layers size={7}/> Expertise</span>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            <div onClick={() => setMobileSpeedOpen(true)}
              style={{ borderRadius:16, background:C.surface, border:'1px solid rgba(255,255,255,0.05)', padding:'20px', display:'flex', flexDirection:'column', justifyContent:'space-between', minHeight:120, cursor:'pointer' }}>
            ...
            {contacts.map(c => (
              <a key={c.id} href={c.href} target="_blank" rel="noopener noreferrer"
                ...
                <div style={{ width:32, height:32, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', background:`${c.brandColor}18`, border:`1px solid ${c.brandColor}28`, flexShrink:0 }}>
                  <svg viewBox="0 0 24 24" fill="currentColor" style={{ width:12, height:12, color:c.brandColor }}><path d={c.path}/></svg>
                </div>
                <span style={{ fontSize:13, color:C.textHi }}>{c.value}</span>
          ...
          return (
            <div key={block.id}
              ...
              style={{
                ...
                transform: !anyActive && dispersed ? `perspective(1200px) rotateX(${mousePos.y*0.035}deg) rotateY(${mousePos.x*-0.035}deg)` : dimmed ? 'scale(0.975)' : 'none',
              ...
              {/* LOGO */}
              {block.isLogo && (<>
                <div style={{ position:'absolute', inset:0, pointerEvents:'none', background:`radial-gradient(circle at ${logoGlint.x}% ${logoGlint.y}%, rgba(200,169,122,0.22) 0%, transparent 55%)`, transition:'background 0.1s ease' }} />
              ...
              {block.isContact && (<>
                <div style={{ position:'absolute', top:-24, right:-24, pointerEvents:'none', zIndex:0 }}>
                  {contacts.map((c,idx) => (
                    <svg key={c.id} viewBox="0 0 24 24" fill="currentColor" style={{ position:'absolute', top:0, right:0, width:160, height:160, color:c.brandColor, opacity: currentContact===idx ? 0.07 : 0, transform: currentContact===idx ? 'scale(1) rotate(0deg)' : 'scale(0.85) rotate(6deg)', transition:'all 1s ease' }}>
                      <path d={c.path}/>
                    </svg>
                  ))}
                </div>
                <div style={{ position:'relative', zIndex:10, height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'2.5vw' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                      <svg viewBox="0 0 24 24" fill="currentColor" style={{ width:13, height:13, color:contacts[currentContact].brandColor, flexShrink:0, transition:'color 0.7s' }}><path d={contacts[currentContact].path}/></svg>
                      <p style={{ fontFamily:"'DM Mono',monospace", fontSize:9, letterSpacing:'0.1em', textTransform:'uppercase', color:C.textMid, margin:0 }}>{contacts[currentContact].value}</p>
                    </div>
                  </div>
                </div>
              </>)}
              ...
              {/* ABOUT */}
              {block.isAbout && (
                <div style={{ height:'100%', display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'1.8vw' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <span style={{ ...pill, fontSize:7, padding:'2px 8px' }}><Quote size={6}/> About</span>
                    <ArrowUpRight size={13} style={{ color:'rgba(255,255,255,0.25)' }} />
                  </div>
                  <div>
                    <p style={{ fontFamily:"'Godber',sans-serif", fontSize:'1.45vw', color:C.textHi, lineHeight:1.25, margin:0 }}>We build<br/>fast. We<br/>build right.<br/>That's it.</p>
      ...
      {activeBlock && (
        <div className="popup-overlay">
          <div className="popup-backdrop" onClick={closeAll} />
            ...
            <div style={{ display:'flex', minHeight:'55vh' }}>
              <div style={{ width:'36%', padding:40, borderRight:'1px solid rgba(255,255,255,0.05)', display:'flex', flexDirection:'column', justifyContent:'space-between', background:'rgba(0,0,0,0.25)', flexShrink:0 }}>
                <div>
                  ...
                  <h3 style={{ fontFamily:"'Godber',sans-serif", fontSize:48, lineHeight:1.05, color:C.textHi, margin:0 }}>{activeBlock.title || 'Details'}</h3>
              ...
              <div style={{ flex:1, padding:40, display:'flex', alignItems:'center', overflowY:'auto' }}>
                <div className="animate-fade-up" style={{ width:'100%' }}>
                  <PopupContent blockId={activeBlock.id} expandedDemo={expandedDemo} setExpandedDemo={setExpandedDemo} />
                </div>
              </div>
            </div>
          </div>
      ...
      {mobileSpeedOpen && (
        <div style={{ position:'fixed', inset:0, zIndex:300, display:'flex', alignItems:'flex-end' }}>
          ...
          <div style={{ position:'relative', width:'100%', background:'#0e0e0e', border:'1px solid rgba(255,255,255,0.08)', borderRadius:'24px 24px 0 0', padding:'32px 24px 48px', animation:'fadeUp 0.35s cubic-bezier(0.19,1,0.22,1) both' }}>
            ...
            <p style={{ fontSize:14, color:C.textDim, lineHeight:1.75, margin:0 }}>From brief to live site in a single business day. No compromises on quality — we've built the systems to move fast without breaking things.</p>
